<script lang="ts">
  ...
  import type { StudentPostulant } from '$lib/types';
  import { getIngressStatus, getMajor, getSemester } from '$lib/utils';

  export let student: StudentPostulant;
</script>
  ...
  <div class="w-full flex flex-col justify-between h-full">
    ...
    <div class="mt-auto">
      <div class="flex space-x-2">
        ...
        <div
          class={student.candidateDIDES
            ? 'py-2 text-md px-3 rounded-full bg-green-200 text-green-800'
            : 'py-2 text-md px-3 rounded-full bg-red-200 text-red-800'}
        >
          DIDES
        </div>
        {#if student.candidateSchool}
          <div
            class="py-2 text-md px-3 bg-indigo-200 text-indigo-800 rounded-full"
          >
            {student.candidateSchool}
          </div>
        {/if}
        {#if student.candidateFaculty}
          <div
            class="py-2 text-md px-3 bg-indigo-200 text-indigo-800 rounded-full"
          >
            {student.candidateFaculty}
          </div>
        {/if}
      </div>
    </div>
...
{#if showInfoModal}
  ...
  >
    <div class="shadow overflow-hidden sm:rounded-lg">
      ...
      {#if student.firedRules && !(student.candidateCU && student.candidateDIDES)}
          ...
          <table class="table-fixed w-full py-2">
            ...
            <tbody>
              {#each student.firedRules as firedRule}
                {#if firedRule.name !== 'Asignación de Escuela y Facultad'}
                  <tr>
                    <td
      ...
      {/if}
      {#if student.sanctions.length > 0}
        <p class="mt-1 max-w-2xl text-md text-black font-bold">Sanciones</p>
        <table class="table-fixed w-full py-2">
          <thead>
            <tr>
              <th class="text-sm font-medium text-gray-500">Código</th>
              <th class="text-sm font-medium text-gray-500">Fecha de Inicio</th>
              <th class="text-sm font-medium text-gray-500">Fecha Fin</th>
            </tr>
          </thead>
          <tbody>
            {#each student.sanctions as sanction}
              <tr>
                <td
                  class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2 text-center"
                  >{sanction.sanctionCode}</td
                >
                <td
                  class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2 text-center"
                  >{moment(sanction.startDate).format('DD/MM/YYYY')}</td
                >
                <td
                  class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2 text-center"
                  >{sanction.endDate
                    ? moment(sanction.endDate).format('DD/MM/YYYY')
                    : ''}</td
                >
              </tr>
            {/each}
